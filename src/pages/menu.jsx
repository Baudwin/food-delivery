...
<div className="products lg:col-span-9 xl:col-span-9  border py-10 px-5 shadow-md">
...
<button onClick={()=>AddToCart(item)} className=" shadow-xl px-2 py-1 text-white flex gap-1 bg-amber-500 rounded-full"> 
...
<div className="grid lg:grid-cols-4 xl:grid-cols-4 md:grid-cols-2 sm:grid-cols-2 gap-2">

{ allItems?.data?.map(item=>{
  return <div key={item._id} className="">
<img  className=" shadow-sm h-56 w-full object-cover border" src={`http://localhost:3005/${item?.img}`} alt="img" />
<h1 className="font-semibold text-md">{item.itemName}</h1>
<div className="flex justify-between items-center">
<p className="text-gray-600 ">{item.price} CFA</p>
<button onClick={()=>AddToCart(item)} className=" shadow-xl px-2 py-1 text-white flex gap-1 bg-amber-500 rounded-full"> 
<span className="text-sm font-medium uppercase">
...
<div className="lg:col-span-3 space-y-4 xl:col-span-3 border md:row-start-1 sm:row-start-1 xsm:row-start-1 xxsm:row-start-1 shadow-md px-2 py-2">
  ...
  <div className="p-2">
  <div className="flex">
    <input className=" border leading-tigh bg-red-50 focus:outline-none w-full px-2 py-2" placeholder="Search item" type="text" name="query" onChange={search} id="" />
    <button className="bg-amber-500 text-white p-2">
